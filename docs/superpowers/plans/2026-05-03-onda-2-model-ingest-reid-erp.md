...
## ✅ Discovery executado em 2026-05-11 — premissas validadas

**Hardware testado:** DH-IPC-HFW5442T-ASE serial `BE06DE8PAG383E6`, firmware `3.142.0000000.8.R` (build 2025-08-26), processador `AX620D`.

| # | Premissa | Status real |
|---|---|---|
| P1 | `eventManager.cgi attach` entrega eventos | ✅ **Confirmada** — `Code=FaceDetection` chega via multipart com `action=Start`/`Stop` |
| P2 | Atributos Age/Gender/Emotion no payload | ✅ **Confirmada com ressalva** — atributos vêm em `data.Object.*` (nested), NÃO em `data.*` top-level |
| P3 | Face DB embarcado via CGI | ❌ **REFUTADA** — `FaceInfoManager.cgi`, `FaceDB.cgi`, `face.cgi`, `IntelliVideoAnalyse.cgi`: todos retornam HTTP 501 Not Implemented |
| P4 | Eventos com `FaceID` para faces cadastradas | ❌ **REFUTADA (consequência de P3)** — sem Face DB, não há cadastro pra reconhecer |
| P5 | Snapshot por demanda em `/cgi-bin/snapshot.cgi?channel=1` | ✅ **Confirmada** — JPEG retorna em ~700ms |

### Payload real do `Object.*` (em FaceDetection events)

```json
{
  "Action": "Appear",
  "Age": 49,
  "Sex": "Man",                    // string ("Man"|"Woman")
  "Gender": 2,                     // numérico (1|2)
  "Emotion": "Confused",           // observados: "Confused", "Neutral" (lista completa: provável Happy/Sad/Angry/Surprise/Disgust/Fear/Calm/Confused)
  "Express": 10,                   // intensidade
  "Beard": 2, "Glass": 1, "Mask": 1, "Mouth": 1, "Eye": 2, "Complexion": 1, "Attractive": 48,
  "Confidence": 255,
  "FaceQuality": 72,               // 0-100, útil pra filtrar detecções ruins
  "BoundingBox": [2576, 3528, 2808, 3992],   // escala fixed-point ~8192 (NORMALIZAR!)
  "Angle": [0, 10, 0],             // [pitch, yaw, roll]
  "ObjectID": 47898,               // estável dentro da sessão de tracking
  "Feature": ["Confused", "NoGlasses"]
}
```

**Outras descobertas operacionais:**
- `action=Start` dispara quando face entra em frame; `action=Stop` quando sai
- BoundingBox em escala fixed-point — **dividir por 8192 e multiplicar pela resolução real do snapshot** para coordenadas de pixel
- `RealUTC` é o timestamp confiável (`UTC` está em fuso da câmera)
- 0 ocorrência de keys tipo `FaceID` confirmando que essa câmera **só faz Face Detection, não Face Recognition** (sem Face DB embarcado)

## ⚠ Adaptação: Caminho C escolhido (MVP enxuto agora + reid local na Onda 3)

Com P3+P4 refutadas, **Re-id estratégia A não é viável** neste hardware. Adaptação:

| Antes do Discovery | Depois (caminho C) |
|---|---|
| Chunk 3: Re-id A com Face DB câmera | **CHUNK 3 REMOVIDO** — adiado para Onda 3 como failover B (InsightFace local + pgvector ANN) |
| Chunk 4 task 4.3: sync funcionários **+ upload pra Face DB câmera** | Sync apenas no DB local (sem upload câmera) |
| Chunk 4 task 4.4: `uploadEmployeePhoto` | **REMOVIDA** — sem Face DB pra subir |
| Chunk 4 task 4.9: `upload-back` após match | **REMOVIDA** — sem Face DB pra subir |
| Task 2.8 normalizer: lê `data.Age` etc | Lê `data.Object.Age` etc (nested) |
| Task 2.7 CanonicalEvent: 4 atributos | Expande pra incluir Beard/Glass/Mask/Mouth/Confidence/FaceQuality/Angle |

**Onda 2 reduzida** de 25 para ~21 tasks. Frontend de admin do Face DB câmera (Task 3.4) tampouco se aplica — fica como diretório vazio.

**Implicações no produto MVP:**
- ✅ Atributos de cada face captada em tempo real (idade, gênero, emoção, qualidade)
- ✅ Sessões agregadas por `ObjectID` enquanto pessoa está visível
- ✅ Clientes identificados via match temporal com checkin do ERP
- ❌ **Cliente recorrente sem checkin no ERP fica anônimo a cada visita** (resolvido na Onda 3 com failover B)
- ❌ **Funcionários sem reconhecimento facial automático** — cadastro fica no DB local mas câmera não os identifica (resolvido na Onda 3)
...
⚠ **Atualizado pós-Discovery (2026-05-11):** o payload Dahua real coloca atributos em `data.Object.*` (nested), não em `data.*` top-level. Os fixtures dos testes abaixo usam payloads **reais** capturados em `discovery-output/run-2026-05-11T16-32-34-520Z/events-*.ndjson`. Não improvise — copie os fixtures literalmente.

Constante de normalização do BoundingBox: Dahua entrega coordenadas em escala fixed-point (~8192). Normalizar para 0-1 dividindo por **8192** (validado empiricamente; pode precisar ajuste fino se aparecerem valores fora de 0-1 em produção).

- [ ] **Step 1: TDD RED — testes com fixture real**
...
describe("normalize (Dahua nested payload)", () => {
  ...
  test("FaceDetection action=Stop vira face.detected.stop", () => {
    const ev = normalize(
      {
        ...realFixture,
        parsed: { ...realFixture.parsed!, action: "Stop" },
      },
      cameraId,
    );
    ...
    const ev = normalize(
      {
        ...
        parsed: {
          ...realFixture.parsed!,
          data: {
            ...realFixture.parsed!.data,
            Object: { ...realFixture.parsed!.data.Object, Sex: "Woman" },
          },
        },
      },
      cameraId,
...
function extractAttrs(obj: Record<string, unknown>): FaceAttributes {
  ...
  if (Array.isArray(obj["Angle"]) && obj["Angle"].length === 3) {
    ...
    if (Number.isFinite(pitch)) attrs.pitch_deg = pitch;
    if (Number.isFinite(yaw)) attrs.yaw_deg = yaw;
    if (Number.isFinite(roll)) attrs.roll_deg = roll;
  }
...
export function normalize(raw: CapturedEvent, cameraId: string): CanonicalEvent | null {
  ...
  // face_attrs sempre presente em FaceDetection (palpável que sempre tem ao menos algo)
...
- [ ] **Step 3: Adicionar 1 fixture extra com `Sex: "Woman"`**

Discovery teve só "Man" — convém pelo menos 1 fixture fabricado pra confirmar mapeamento "Woman" → "female". Já incluído nos testes acima.

⚠ **Calibração do `BBOX_FIXED_POINT_SCALE`:** se em produção aparecerem bbox.x ou bbox.y > 1.0 ou < 0.0, ajustar a constante. Validar comparando com snapshot real (largura do bbox em pixels deve bater com a face real na imagem).
...
## Chunk 3: REMOVIDO (caminho C — Re-id A não viável)

**Decidido após Discovery em 2026-05-11.** Premissas P3+P4 refutadas (câmera
DH-IPC-HFW5442T-ASE não tem Face DB embarcado via CGI; ver `## ✅ Discovery
executado em 2026-05-11` no topo do plano para detalhes).

As 5 tasks originais (3.1 Face DB client, 3.2 REST endpoints, 3.3 reid-mgr
lookup, 3.4 frontend admin, 3.5 verificação) **não se aplicam** a este hardware.
Reconhecimento facial entre sessões fica para **Onda 3 — failover B**:
sidecar Python com InsightFace + pgvector ANN match em snapshots locais.

### Implicações para o pipeline (Chunk 2)

- `face_records.camera_face_id` permanece NULL para todas as detecções na Onda 2
- `face_records.embedding` permanece NULL — populado apenas em Onda 3
- Cada detecção criada por `processEvent()` cria uma `Person` anônima na primeira
  vez que aquele `track_id` aparece, ou reaproveita a sessão atual via
  `findOpenForTrack` se gap < 30s. **Não há reconhecimento entre sessões na Onda 2.**
- Cliente que volta amanhã: vira nova `Person` anônima, a menos que faça
  checkin no ERP dentro da janela ±5min (Chunk 4 vincula via match temporal)

### Critério para reconsiderar (futuro)

Se um firmware Dahua mais novo expor Face DB CGI (improvável neste modelo —
AX620D processor sem licença AI Smart Recognition), reabrir esta seção.
Caso contrário, Onda 3 entrega failover B como **estratégia única** de re-id.
...
### Task 4.3: Sync funcionários — cache no DB local

> ⚠ **Pós-Discovery 2026-05-11:** este plano originalmente upload-ava as fotos pro Face DB
> embarcado da câmera (Task 4.4). Como a câmera não tem Face DB CGI (P3 refutada), a Task 4.3
> agora **só popula o cache local** (`erp_employees` + `Person` records). Reconhecimento
> facial automático de funcionários só funciona em Onda 3 com failover B (InsightFace local).
...
### Task 4.4: REMOVIDA (Face DB câmera não viável)

**Decidido após Discovery em 2026-05-11.** O upload de foto de funcionário ao
Face DB embarcado não se aplica a este hardware (P3 refutada). Funcionários
ficam apenas em `erp_employees` cache local + correspondente `Person` no DB.

**Reconhecimento facial automático de funcionários** (sem precisar do ERP)
depende do failover B (InsightFace local) — vai pra **Onda 3**. Na Onda 2,
funcionários são identificados apenas se passarem por algum evento que os
vincule (ex: registro manual via UI futura, ou cadastro proativo de embedding
local em Onda 3).
...
### Task 4.9: REMOVIDA (Face DB câmera não viável)

**Decidido após Discovery em 2026-05-11.** O "upload-back" da face anônima após
auto-match (que faria a próxima visita do cliente vir já reconhecida pela câmera)
não se aplica — câmera não tem Face DB embarcado (P3 refutada).

**Implicação prática:** mesmo após o cliente ser auto-vinculado a um checkin do
ERP via match temporal, na próxima visita o cliente **vira anônimo de novo** até
fazer outro checkin. Reconhecimento entre visitas independente do ERP fica para
Onda 3 com failover B (InsightFace embedding salvo em `face_records.embedding`).

**Workaround Onda 2:** o `match_attempts` registra o vínculo histórico, então
o frontend (Onda 3) pode mostrar "este anônimo já foi cliente João Silva em N
visitas anteriores" via JOIN entre `match_attempts` e checkins do ERP.
